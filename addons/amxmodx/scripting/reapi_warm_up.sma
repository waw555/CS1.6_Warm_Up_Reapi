...
#define TASK_WARMUP_RESTART 34000
...
new g_iPlayerBaseAward[MAX_PLAYERS + 1];
new g_iPlayerLeaderKillAward[MAX_PLAYERS + 1];
new g_iMaxPlayers;
...
// Убирает ошибочную метку VIP в TAB-таблице (ScoreAttrib), сохраняя dead/bomb.
public MessageScoreAttrib(msgid, dest, id)
{
	new iAttrib = get_msg_arg_int(2);

	if (!(iAttrib & SCORE_ATTRIB_VIP))
		return PLUGIN_CONTINUE;

	iAttrib &= ~SCORE_ATTRIB_VIP;

	if (iAttrib & SCORE_ATTRIB_DEAD)
		iAttrib |= SCORE_ATTRIB_DEAD;

	if (iAttrib & SCORE_ATTRIB_BOMB)
		iAttrib |= SCORE_ATTRIB_BOMB;

	set_msg_arg_int(2, ARG_BYTE, iAttrib);
	return PLUGIN_CONTINUE;
}

// Принудительно снимает VIP-статус с игрока (если он был выставлен другим кодом).
stock ClearPlayerVipStatus(id)
{
...
// Снимает VIP-статус со всех подключенных игроков.
stock ClearVipStatusForAll()
{
	new iPlayers[MAX_PLAYERS], iNum;
	get_players(iPlayers, iNum, "h");

	for (new i; i < iNum; i++)
		ClearPlayerVipStatus(iPlayers[i]);
}
...
public CBasePlayer_Killed(Victim, Attacker, gib)
{
	...
	if (!g_bLeaderModeEnabled)
	{
		...
		AddWarmupReward(Attacker, g_iKillReward);
		g_iPlayerBaseAward[Attacker] += g_iKillReward;
	...
	new bool:bIsFirstKillEvent = bFirstLeaderKill && !g_bFirstKillHappened;

	if (!bLeaderExists && g_iCurrentLeaderKillReward <= 0)
		g_iCurrentLeaderKillReward = g_iLeaderKillRewardStart;

	g_iPlayerKills[Attacker]++;
	if (!bIsFirstKillEvent)
	{
		AddWarmupReward(Attacker, g_iKillReward);
		g_iPlayerBaseAward[Attacker] += g_iKillReward;
	}
	...
	if (bLeaderKilled || bFirstLeaderKill)
	{
		...
		if (iLeaderReward > 0)
		{
			AddWarmupReward(Attacker, iLeaderReward);
			g_iPlayerLeaderKillAward[Attacker] += iLeaderReward;
		}
...
// Формирует список иконок оружия для StatusIcon на основе набора оружия разминки.
...
// Преобразует токен оружия в имя соответствующего HUD-спрайта.
...
// Показывает иконки оружия разминки всем подключенным игрокам.
...
// Скрывает иконки оружия разминки у всех подключенных игроков.
...
// Показывает иконки оружия разминки конкретному игроку.
...
// Скрывает иконки оружия разминки у конкретного игрока.
...
// Выводит DHUD-блок с текущим лидером и наградой за его убийство.
...
// Выполняет дополнительный рестарт раунда по таймеру и останавливает музыку.
public Task_WarmupRestart()
{
...
{
	for (new i; i < sizeof(g_szDefaultConfig); i++)
		write_file(g_szWarmUpConfigPath, g_szDefaultConfig[i], -1);
}
...
// Парсит цвет спрайта оружия разминки из строки R G B.
...
// Инициализирует HUD/деньги разминки для игрока, зашедшего во время warmup.
...
// Накладывает затемнение экрана игроку на период показа итогов.
...
// Блокирует атаки игроков во время показа итогов разминки.
...
public CSGameRules_RestartRound_Post()
{
	...
	new iPlayers[MAX_PLAYERS], iNum, iPlayer;
	new iStartMoney = get_pcvar_num(get_cvar_pointer("mp_startmoney"));
	...
	for(new i=0; i < iNum; i++)
	{
		...
		if (iDelta > 0)
			...
			SetMoneyNoReward(iPlayer, iTargetMoney);
...
public ShowStats()
{
	...
	g_arrData[g_iPlayerTop][AWARD] = iBonusAward;
	...
	switch(g_iCounter)
	{
		...
		case 4:
		{
			...
			show_dhudmessage(0, "%d: %s - УБИЙСТВА - %d | УРОН - %d | НАГРАДА - %d$", iCount, szName, iKills, iDamage, iBonusAward);
...
// Завершает разминку, возвращает серверные cvar и запускает рестарт(ы) раунда.
stock FinishWarmupAndRestart()
{
	...
	if (g_pCvar[RESTART] > 1)
		set_task(1.5, "Task_WarmupRestart", TASK_WARMUP_RESTART, .flags = "a", .repeat = g_pCvar[RESTART] - 1);
...
// Применяет здоровье разминки всем живым игрокам.
...
// Устанавливает одинаковое здоровье всем живым игрокам.
...
// Обнуляет статистику разминки у всех подключенных игроков.
...
// Сбрасывает frags/deaths на табло для всех игроков.
...
// Обнуляет деньги разминки у всех подключенных игроков.
stock ResetWarmupMoneyForAll()
{
	...
	for (new i; i < iNum; i++)
		SetMoneyNoReward(iPlayers[i], 0);
}

// Устанавливает деньги игроку без зеленого индикатора денежной награды.
stock SetMoneyNoReward(id, money)
{
	if (!IsPlayer(id) || !is_user_connected(id))
		return;
...
// Начисляет промежуточную награду разминки и сразу синхронизирует HUD денег.
stock AddWarmupReward(id, amount)
{
	if (amount <= 0 || !IsPlayer(id) || !is_user_connected(id))
		return;
...
// Добавляет игроку сумму денег с обычной серверной анимацией награды.
